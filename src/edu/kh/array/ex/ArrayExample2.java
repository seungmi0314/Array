...
public class ArrayExample2 {
	...
	public void createLottonNumber() {
		//로또번호 생성기
		// [6,12,16,20,21,42]
		
		// 1. 1~45 사이 중복되지 않는 난수 6개 생성 => Mathe.random()
		// 2. 생성된 난수가 오름차순으로 정렬 => Arrays.sort()
		
		// 1) 정수 6개를 저장 할 배열 선언 ㅣㅊ 할당
		int[] lotto = new int[6];
		...
		for (int i = 0; i < lotto.length; i++) {
			...
			//5) 중복검사를 위한 for문 작성
			for(int x = 0; x < i; x++) {
				//왜 i까지만 돌면 되냐면 생성된 애들까지만 검사하면 돼가지구
				// 6) 현재 생성된 난수와 같은 수가
				// 앞쪽 요소에 있는지검사
				if(random == lotto[x]) {
					...
					// -> i는 기본적으로 0~5까지 6회 반복되지만 i값을 인위적으로 1감소시켜
					//    7회 반복되는 모양을 만듦.
					break;
					//앞쪽에서 중복 데이터 발경하면 남은값을 비교할 필요 없음
